...
# 7. Add economic factor in prestige
...
def dynamic_status(t, s_base, gdp_df, lang_to_countries, alpha=0.5):

    e_norm = compute_ei_t(gdp_df, lang_to_countries, t)

    s = alpha * s_base + (1 - alpha) * e_norm
...
def main(filename):
    ...
    if not isinstance(s_fit, np.ndarray):
        s_fit = np.array(s_fit)

    if not np.issubdtype(s_fit.dtype, np.floating):
        raise TypeError(f"s_fit must be an array of floats, but got dtype {s_fit.dtype}")
    # Dynamic status where GDP affects si
    s_dynamic = lambda t: dynamic_status(t, s_fit, gdp_df, lang_to_countries)
    
    # Single simulation (for clean plot)
    x0 = proportions[:, -1]
    x_forecast = simulate_language_dynamics(
        x0, s_dynamic, a_fit, beta_fit,
    ...
    plt.title('Language Projections with Dynamic Status Changes')
    plt.xlabel('Year')
